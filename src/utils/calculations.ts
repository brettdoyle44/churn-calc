...
export function calculateAnnualRevenueLost(inputs: CalculatorInputs): number {
  const { numberOfCustomers, churnRate, averageOrderValue, purchaseFrequency } =
    inputs;
 ...
 * ```
 */
export function calculateLifetimeValueLost(
...
export function calculateAllResults(
  ...
  if (!inputs || inputs.numberOfCustomers <= 0) {
    throw new Error(
      "Invalid calculator inputs: numberOfCustomers must be greater than 0"
    );
  }
  ...
  const customersLostPerYear =
    inputs.numberOfCustomers * (inputs.churnRate / 100);
  const customersLostPerMonth = customersLostPerYear / 12;

  // Generate churn reduction scenarios
  const churnReductionScenarios = calculateChurnReductionScenarios(
    inputs,
    annualRevenueLost
  );
